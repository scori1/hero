...
case 'bc':
case 'bcbut':
if (!isOwner && !fromMe) return reply(`Hanya Untuk @${ownerNumbers.split("@")[0]}`)
if (args.length < 1) return reply('Teksnya?')
anu = await zero.chats.all()
for (let _ of anu) {
buttonss = [{buttonId: `Owner`, buttonText: {displayText: '🗣️ Owner'}, type: 1}]
var btnbc = {
contentText: `${q}`,
buttons: buttonss,
headerType: 1
...
costum(buffer, sticker, Verived, `© - GiiXyz`)
fs.unlinkSync(media)
fs.unlinkSync(ran)
})
...
buffer = fs.readFileSync(ran)
costum(buffer, image, Verived, `© - GiiXyz`)
fs.unlinkSync(ran)
})
break
case 'owner':
